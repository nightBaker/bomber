    ...
    public class Board
    {
        ...
        public string ToString()
        {
           return string.Format("{0}\n" +
                    ...
                    ListToString(GetBombs().Select(b=>b.Point).ToList()),
        ...
        public List<Point> GetBarrier()
        {
            return GetMeatChoppers()
                .Concat(GetWalls())
                .Concat(GetBombs().Select(p=>p.Point))
        ...
        {
            return getBombs(Element.BOMB_TIMER_1,1)
                .Concat(getBombs(Element.BOMB_TIMER_2,2))
                .Concat(getBombs(Element.BOMB_TIMER_3,3))
                .Concat(getBombs(Element.BOMB_TIMER_4,4))
                .Concat(getBombs(Element.BOMB_TIMER_5,5))
                .Concat(getBombs(Element.BOMB_BOMBERMAN,6))
                .Concat(getBombs(Element.OTHER_BOMB_BOMBERMAN,6))
                .ToList();

            List<Bomb> getBombs(Element element, int ticks)
            {
                return Get(element).Select(bombElement => new Bomb
                {
                    Point = bombElement,
                    Ticks = ticks
                }).ToList();
        ...
        public List<Point> GetFutureBlasts()
        {
            ...
            foreach (var bomb in bombs)
            {
                ...
                result.Add(bomb.Point.ShiftLeft());
                result.Add(bomb.Point.ShiftRight());
                result.Add(bomb.Point.ShiftTop());
                result.Add(bomb.Point.ShiftBottom());
            }
